...
async function authenticate(req, res, next) {
  try {
    ...
    if (cachedUser) {
      // ✅ THÊM: Kiểm tra user vẫn còn active trong DB
      const currentUser = await db.getUserByEmail(cachedUser.email);
      
      if (!currentUser || currentUser.id !== cachedUser.id) {
        console.log(`⚠️ Cache mismatch: User ${cachedUser.email} đã bị xóa hoặc thay đổi ID`);
        userCache.del(token); // Xóa cache cũ
      } else {
        req.user = currentUser; // Dùng data mới nhất từ DB
        return next();
      }
    }
